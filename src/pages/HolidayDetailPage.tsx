  ...
  if (!holiday) {
    return (
      <div className="container py-16 text-center pb-mobile-nav">
        ...
        <Link to="/holidays" className="mt-4 inline-block text-accent hover:underline">← Back to calendar</Link>
      </div>
    );
  }
  ...
  return (
    <div className="pb-mobile-nav">
      ...
      <div className="container pt-6 pb-12 max-w-2xl">
        {/* Breadcrumb */}
        <Link to="/holidays" className="inline-flex items-center gap-1 text-label text-muted-foreground hover:text-foreground transition-colors mb-4">
        ...
        <div className="mt-8 flex items-center justify-between gap-3">
          {prev ? (
            <Link to={`/holidays/${prev.slug}`} className="text-label text-muted-foreground hover:text-foreground transition-colors">
              ← {prev.name}
            </Link>
          ) : <span />}
          {next ? (
            <Link to={`/holidays/${next.slug}`} className="text-label text-muted-foreground hover:text-foreground transition-colors">
              {next.name} →
            </Link>
          ) : <span />}
        </div>
